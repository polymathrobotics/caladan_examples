...
layout = [ 
            ...
            [sg.Graph(canvas_size=(0.001248654*scale, 0.001578659*scale), graph_bottom_left=(0, 0), graph_top_right=(0.001248654*scale, 0.001578659*scale), background_color='#1C1E23', enable_events=True, key='graph')],
            [sg.Button('Start Parking'),sg.Button('STOP',button_color = ('white','red')),sg.Button('Train Load'),sg.Button('Train Unload')],
            [sg.Button('Return to Equipment Shed'),sg.Button('Crane A'),sg.Button('Crane B'),sg.Button('Crane C')],
         ]


window = sg.Window("Polymath Robotics Caladan Portworld Example", layout, icon='icon.png',finalize=True)
graph = window['graph'] 
...
def update_loop():
    while True:
        ...
        portworld_config.odom["orientation"] = math.atan2(2.0 * (quat["w"] * quat["z"]),1.0 - 2.0 * (quat["z"]*quat["z"]))
        window.write_event_value('-UPDATE-','updated')

def tolerance_check(goal,tol):
    if abs(portworld_config.odom["position"]["latitude"]-goal[0])<tol and abs(portworld_config.odom["position"]["longitude"]-goal[1])<tol:
        return False
    else:
        return True
...
def clear_image():
    graph.Erase()
    image = graph.DrawImage(filename="port_world.png", location=(0, 0.001578659*scale-1))
...
while True:
    ...
    if window['Connect'].get_text() == "Connected": # Only update if connected
        ...
        window['-VEL-'].update(str(round(portworld_config.odom["odometry"]["linear.x"],2)) + " m/s  ")
        current_pose = "lat: " + str(round(portworld_config.odom["position"]["latitude"],6)) + " lon: " + str(round(portworld_config.odom["position"]["longitude"],6)) + " angle: " + str(round(portworld_config.odom["orientation"],3))
        window['-POSE-'].update(current_pose)
        draw_pose = (scale*(37.72600802278773-portworld_config.odom["position"]["latitude"]),-scale*(-121.00031316114797-portworld_config.odom["position"]["longitude"]))
        position = graph.draw_circle(draw_pose, 3, fill_color='#1C1E23', line_color='white')
    ...
    if event == 'Connect':
        ...
        if "uuid" in uuid_response:
            window['-PROMPT-'].update("Successfully connected to vehicle " + uuid_response["uuid"])
    ...
    if event == 'Return to Equipment Shed' and window['Connect'].get_text() == "Connected":
        clear_image()
    ...
    if event == 'graph' and window['Connect'].get_text() == "Connected":
        clear_image()
        x, y = values['graph'] 
        lat,lon,yaw = 37.72600802278773-x/scale,-121.00031316114797-y/-scale,0 #NOTE: Clicking on the map always sends orientation 0!
        lat_lon_label = graph.DrawText(text=(round(lat,5),round(lon,5)), location=(x, y-7),font=font,color='white')
