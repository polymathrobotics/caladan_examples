...
layout = [ 
            ...
            [sg.Button('Start Tilling'),sg.Button('STOP',button_color = ('white','red')),sg.Text('',key='-STATUS-',size=(45,None))],
            [sg.Button('Return to Equipment Shed')],
         ]


window = sg.Window("Polymath Robotics Caladan Farmworld Example", layout, icon='icon.png',finalize=True)
graph = window['graph'] 
...
def send_goal(lat,lon,yaw):
    ...
    target_text = graph.DrawText(text=(round(lat,5),round(lon,5)), location=(x, y-7),font=font,color='white')
...
def update_loop():
    while True:
        ...
        farmworld_config.odom["orientation"] = math.atan2(2.0 * (quat["w"] * quat["z"]),1.0 - 2.0 * (quat["z"]*quat["z"]))
        window.write_event_value('-UPDATE-','updated')
        time.sleep(1.2)

def tolerance_check(goal,tol):
    if abs(farmworld_config.odom["position"]["latitude"]-goal[0])<tol and abs(farmworld_config.odom["position"]["longitude"]-goal[1])<tol:
        return False
    else:
        return True
...
def graph_clean():
    graph.Erase()
    image = graph.DrawImage(filename="farm_world.png", location=(0, 0.001578659*scale-1))
...
while True: # Main UI Loop
    ...
    if window['Connect'].get_text() == "Connected": # Only update and accept other commands if already connected
        window['-VEL-'].update(str(round(farmworld_config.odom["odometry"]["linear.x"],2)) + " m/s  ")
        current_pose = "lat: " + str(round(farmworld_config.odom["position"]["latitude"],6)) \
            + " lon: " + str(round(farmworld_config.odom["position"]["longitude"],6)) \
            + " angle: " + str(round(farmworld_config.odom["orientation"],3))
        window['-POSE-'].update(current_pose)
        window['-STATUS-'].update(farmworld_config.stat)
        draw_pose = latlon_to_pixelXY(farmworld_config.odom["position"]["latitude"],farmworld_config.odom["position"]["longitude"])
        ...
        if event == 'graph':
            ...
            send_goal(lat,lon,yaw=0) #NOTE: Clicking on the map always sends orientation 0!
    ...
    if event == 'Connect': # First time clicking the connect button, check we get UUID
        ...
        if "uuid" in uuid_response:
            ...
            window['-PROMPT-'].update(uuid_response) #otherwise, print returned error message
